                    ...
                    <tbody class="text-gray-700">
                        @foreach ($data as $entry)
                            <tr class="border-b hover:bg-gray-100">
                                <td class="py-3 px-4">{{ \Carbon\Carbon::parse($entry->timestamp)->format('M d, Y') }}</td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (energyData.length > 0) {
                const labels = energyData.map(data => new Date(data.timestamp).toLocaleDateString('en-US', {
                    month: 'short',
                    day: 'numeric'
                }));
